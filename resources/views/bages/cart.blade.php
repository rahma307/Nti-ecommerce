...
@forelse ($cartProducts->take(4) as $product)
    ...
    <form action="{{ route('cart.toggle', $product->id) }}" method="POST"  class="cart-action-form" >
        @csrf
        <input type="number" min="1" name="decrease" value="{{ $product->pivot->quantity }}" class="number" />
        <input type="submit" value="Update" class="sub">
    </form>
    <form action="{{ route('cart.remove', $product->id) }}" method="POST"  class="cart-action-form">
        @method('Delete')
        @csrf
